...
The first part we want to create is our player object. Under the variables we declared in the previous step, create a new object representing our player and store it in a variable called ``player``. For now, we will give our player object three properties: ``hitbox``, ``color``, and ``draw``.
...
Remember when we assigned a function to ``document.body.onload`` to run some code when the page loaded? Well, we can edit a similar property to have code whenever a key is pressed on the keyboard. Inside the function we assgined to ``document.body.onload``, add the following snippet:
```JavaScript
